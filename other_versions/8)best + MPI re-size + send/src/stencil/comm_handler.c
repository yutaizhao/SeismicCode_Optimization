...
comm_handler_t comm_handler_new(u32 rank, u32 comm_size, usz dim_x, usz dim_y, usz dim_z) {
    // Compute splitting
    u32 const nb_x = gcd(comm_size, (u32)(dim_y * dim_z));
    u32 const nb_z = gcd(comm_size / nb_x, (u32)dim_x);
    u32 const nb_y = (comm_size / nb_x) / nb_z;
        ...
        for (usz j = 0; j < mesh->dim_y; ++j) {
            switch (comm_kind) {
                case COMM_KIND_SEND_OP:
                    MPI_Send(
                             mesh->cells_value[i][j], mesh->dim_z, MPI_DOUBLE, target, 0, MPI_COMM_WORLD
                             );
                    break;
                case COMM_KIND_RECV_OP:
                    MPI_Recv(
                             mesh->cells_value[i][j],
                             mesh->dim_z,
                             MPI_DOUBLE,
                             target,
                             0,
    ...
    for (usz i = 0; i < mesh->dim_x; ++i) {
        for (usz j = 0; j < mesh->dim_y; ++j) {
            switch (comm_kind) {
                case COMM_KIND_SEND_OP:
                    MPI_Send(
                             &mesh->cells_value[i][j][z_start], STENCIL_ORDER, MPI_DOUBLE, target, 0, MPI_COMM_WORLD
                             );
                    break;
                case COMM_KIND_RECV_OP:
                    MPI_Recv(
                             &mesh->cells_value[i][j][z_start],
                             STENCIL_ORDER,
                             MPI_DOUBLE,
                             target,
                             0,
